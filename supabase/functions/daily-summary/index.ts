...
// Called after a mood check-in is saved.
// Reads biometrics + energy forecast + mood for the day,
// generates a rule-based summary, upserts daily_summaries.
//
...
serve(async (req) => {
  ...
  try {
    ...
    if (!date) return new Response('date required', { status: 400, headers: CORS });

    // Fetch all data in parallel
    const [bioRes, energyRes, moodRes, taskRes, eventRes] = await Promise.all([
      supabase.from('biometric_readings').select('*').eq('user_id', user.id).eq('date', date).single(),
      supabase.from('energy_forecasts').select('*').eq('user_id', user.id).eq('date', date),
      supabase.from('mood_checkins').select('mood_score, notes, energy_impact').eq('user_id', user.id).eq('date', date),
      supabase.from('tasks').select('id, completed').eq('user_id', user.id).eq('date', date),
      supabase.from('calendar_events').select('start_minutes, end_minutes').eq('user_id', user.id).eq('date', date).eq('is_deleted', false),
    ]);
    ...
    const meetingMinutes = events.reduce((sum: number, e: any) => sum + (e.end_minutes - e.start_minutes), 0);
    ...
    const { error: upsertErr } = await supabase
      ...
      .upsert(summaryRow, { onConflict: 'user_id,date' });

    if (upsertErr) throw upsertErr;

    return new Response(
      JSON.stringify({ success: true, summary_text: summaryText, capacity_score: capacityScore }),
...
function computeBiometricBase(bio: any): number {
  if (!bio) return 0.50; // fallback when no data synced yet
...
function buildSummary(p: SummaryParams): string {
  ...
  if (p.capacityScore >= 0.65) {
    ...
    return templates[new Date().getDay() % templates.length];
  }
  ...
  return lowTemplates[new Date().getDay() % lowTemplates.length];
}
